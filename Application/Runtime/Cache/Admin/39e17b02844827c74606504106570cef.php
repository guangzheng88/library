...
<head>
    ...
    <script type="text/javascript">
        function openNew(){
            var options = {
                contentUrl:'<?php echo U("Admin/detail");?>',
                confirmUrl:'<?php echo U("Admin/addAdmin");?>',
                title:'添加管理员',
                formName:'form'
            };
            TPA.dialog(options);
        ...
        function edit( id ){
            var options = {
                contentUrl:'<?php echo U("Admin/detail");?>',
                contentData:{id:id},
                confirmUrl:'<?php echo U("Admin/updateAdmin");?>',
                confirmData:{id:id},
                title:'修改管理员',
                formName:'form'
            };
...
<div class="col-sm-3 col-md-2 sidebar">
    ...
    <ol class="breadcrumb">
        <li><a href="/">首页</a></li>
        <li class="active"><a href="<?php echo U('Admin/adminList');?>">管理员列表</a></li>
    </ol>
        ...
        <table class="table table-striped table-hover">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr class="<?php if(empty($item["enabled"])): ?>danger<?php endif; ?>">
                    ...
                    <td>
                        ...
                        <a href="javascript:enabledSwitch(<?php echo ($item["id"]); ?>, 1-<?php echo ($item["enabled"]); ?>)" title="<?php if(empty($item["enabled"])): ?>解锁<?php else: ?>锁定<?php endif; ?>"><span class="glyphicon glyphicon-off <?php if(empty($item["enabled"])): ?>text-danger<?php else: ?>text-success<?php endif; ?>" aria-hidden="true"></span></a>
                        <a href="<?php echo U('module/assignPower',['id' => $item['id']]);?>" title="分配管理员"><span class="glyphicon glyphicon-asterisk" aria-hidden="true"></span></a>
                    </td>
